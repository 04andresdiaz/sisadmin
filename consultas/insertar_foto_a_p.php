	...
	if (isset($_POST['enviar_foto'])) {
		...
		if (in_array($extencion, $formatos)) {
			if (move_uploaded_file($nombreTemporalArchivo, "../perfiles/$nombreArchivo")) {
				$insertar = $conexion -> query ("UPDATE sisadmin.arrendatarios SET foto = '$nombreArchivo' WHERE arrendatarios.cedula = '$id_usuario'");

				$insertar2 = $conexion -> query ("UPDATE sisadmin.propietarios SET foto = '$nombreArchivo' WHERE propietarios.cedula = '$id_usuario';");
				if ($insertar == true) {
